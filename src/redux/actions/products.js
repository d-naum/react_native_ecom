...
export function getProductVariations(product_id) {
    return (dispatch) => {
        let url = types.getProductVariations()
        dispatch({ type: types.FETCH_PRODUCT_VARIATIONS })
        fetch(url)
            .then(res => res.json())
            .then(json => {
                dispatch({
                    type: types.FETCH_PRODUCT_VARIATIONS_SUCCESS,
                    data: json.filter(x => x.product_id == product_id),
                })
            })
            .catch(err => {
                dispatch({
                    type: types.FETCH_PRODUCT_VARIATIONS_FAILED,
                    data: [],
                    error: err.message
                })
            })
